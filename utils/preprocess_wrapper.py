# Small modifications made to the atari preprocessing wrapper from open ai gym
# and additional methods added to better track environment observations as per the Berkley Assignment.
...
class AtariPreprocessing(gym.Wrapper):
    ...
    * Scale observation: optional
    ...
    def __init__(self, env, noop_max, frame_skip, screen_size, terminal_on_life_loss, grayscale_obs,
                 grayscale_newaxis, scale_obs):
        super().__init__(env)
        ...
        assert noop_max >= 0
        assert env.unwrapped.frameskip == 1
    ...
    def _process_obs(self):
        ...
        # original implementation (INTER_AREA):
        # obs = cv2.resize(self.obs_buffer[0], (self.screen_size, self.screen_size), interpolation=cv2.INTER_AREA)

        # new implementation (INTER_LINEAR):
        obs = cv2.resize(self.obs_buffer[0], (self.screen_size, self.screen_size), interpolation=cv2.INTER_LINEAR)
    ...
    # TODO: Implement rendering self.obs
